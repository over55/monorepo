// File Path: monorepo/web/workery-frontend-deprecated/src/Components/Reusable/FormSelectFieldForServiceFee
...
function FormSelectFieldForServiceFee({
  ...
  const setServiceFeeIDOverride = (serviceFeeID) => {
    ...
    for (let index in serviceFeeOptions) {
      ...
      if (
        serviceFeeOption.label === "Other" &&
        serviceFeeOption.value === serviceFeeID
      ) {
  ...
  return (
    <>
      <div
        className={`field pb-4 ${isHidden && "is-hidden"}`}
        key={serviceFeeID}
      >
          ...
          <span className="select">
            {serviceFeeOptions.length > 0 && (
              ...
              >
                {serviceFeeOptions &&
                  serviceFeeOptions.length > 0 &&
                  serviceFeeOptions.map(function (option, i) {
                    return (
                      <option
                        selected={serviceFeeID === option.value}
                        value={option.value}
                        name={option.label}
                      >
                        {option.label}
