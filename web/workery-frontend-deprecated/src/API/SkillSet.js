// File Path: monorepo/web/workery-frontend-deprecated/src/API/SkillSet.js
import getCustomAxios from "../Helpers/customAxios";
import { camelizeKeys, decamelizeKeys, decamelize } from "humps";
import { DateTime } from "luxon";

import {
  WORKERY_SKILL_SETS_API_ENDPOINT,
  WORKERY_SKILL_SET_API_ENDPOINT,
  WORKERY_SKILL_SET_SELECT_OPTIONS_API_ENDPOINT,
} from "../Constants/API";

export function getSkillSetSelectOptionListAPI(
  onSuccessCallback,
  onErrorCallback,
  onDoneCallback,
  ...
  let aURL = WORKERY_SKILL_SET_SELECT_OPTIONS_API_ENDPOINT;
...
export function getSkillSetListAPI(
  filtersMap = new Map(),
  onSuccessCallback,
  onErrorCallback,
  onDoneCallback,
  ...
  let aURL = WORKERY_SKILL_SETS_API_ENDPOINT;
...
// }
//
export function postSkillSetCreateAPI(
  data,
  onSuccessCallback,
  onErrorCallback,
  onDoneCallback,
  ...
  axios
    .post(WORKERY_SKILL_SETS_API_ENDPOINT, decamelizedData)
    .then((successResponse) => {
      const responseData = successResponse.data;
...
export function getSkillSetDetailAPI(
  organizationID,
  onSuccessCallback,
  onErrorCallback,
  onDoneCallback,
  ...
  const axios = getCustomAxios(onUnauthorizedCallback);
  axios
    .get(WORKERY_SKILL_SET_API_ENDPOINT.replace("{id}", organizationID))
    .then((successResponse) => {
      const responseData = successResponse.data;
...
export function putSkillSetUpdateAPI(
  decamelizedData,
  onSuccessCallback,
  onErrorCallback,
  onDoneCallback,
  ...
  axios
    .put(
      WORKERY_SKILL_SET_API_ENDPOINT.replace("{id}", decamelizedData.id),
      decamelizedData,
    )
    .then((successResponse) => {
      const responseData = successResponse.data;
...
export function deleteSkillSetAPI(
  id,
  onSuccessCallback,
  onErrorCallback,
  onDoneCallback,
  ...
  const axios = getCustomAxios(onUnauthorizedCallback);
  axios
    .delete(WORKERY_SKILL_SET_API_ENDPOINT.replace("{id}", id))
    .then((successResponse) => {
      const responseData = successResponse.data;
