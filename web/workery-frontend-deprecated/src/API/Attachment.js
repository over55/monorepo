// File Path: monorepo/web/workery-frontend-deprecated/src/API/Attachment.js
...
export function postAttachmentCreateAPI(
  ...
  axios
    .post(WORKERY_ATTACHMENTS_API_ENDPOINT, formdata, {
      headers: {
...
export function getAttachmentDetailAPI(
  ...
  const axios = getCustomAxios(onUnauthorizedCallback);
  axios
    .get(WORKERY_ATTACHMENT_API_ENDPOINT.replace("{id}", attachmentID))
    .then((successResponse) => {
      const responseData = successResponse.data;
...
export function putAttachmentUpdateAPI(
  ...
  axios
    .put(WORKERY_ATTACHMENT_API_ENDPOINT.replace("{id}", id), data, {
      headers: {
        "Content-Type": "multipart/form-data",
        Accept: "application/json",
      },
...
export function deleteAttachmentAPI(
  ...
  const axios = getCustomAxios(onUnauthorizedCallback);
  axios
    .delete(WORKERY_ATTACHMENT_API_ENDPOINT.replace("{id}", id))
    .then((successResponse) => {
      const responseData = successResponse.data;
