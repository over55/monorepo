...
function AdminStaffDetailAttachmentAddPage() {
  ...
  const fetchStaffDetail = async () => {
    try {
      setFetching(true);
      // Using staffManager if it exists, otherwise we just proceed without fetching
      if (staffManager && staffManager.getStaffDetail) {
                      ...
                      <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-gray-400 transition-colors">
                        <div className="space-y-1 text-center">
                          ...
                          <p className="text-xs text-gray-500">
                            Any file type up to {MAX_FILE_SIZE / (1024 * 1024)}
                            MB
                          </p>
                        </div>
                      </div>
