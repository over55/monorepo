...
function AdminCustomerDetailAttachmentDeletePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Verify the attachment belongs to this customer
      if (
        attachmentData.customerId !== cid &&
        attachmentData.ownershipId !== cid
      ) {
        setErrors({
          general: "This attachment does not belong to the selected customer",
        });
      }
    } catch (error) {
  ...
  const handleCancel = () => {
    navigate(`/admin/customer/${cid}/attachment/${aid}`);
  };
        ...
        <ol className="inline-flex items-center space-x-1 md:space-x-3">
          ...
          <li>
            <div className="flex items-center">
              <span className="mx-2 text-gray-400">/</span>
              <Link
                to={`/admin/customer/${cid}/attachment/${aid}`}
                className="text-sm font-medium text-gray-700 hover:text-blue-600"
              >
                Detail
              </Link>
