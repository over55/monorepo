...
func TestNewNAICSImporterAtDataDir(t *testing.T) {
	...
	t.Run("DataDirectoryIsIncorrect", func(t *testing.T) {
		_, err := NewNAICSImporterAtDataDir("./data2")
		if err == nil {
...
func TestNAICSImporter_ImportByVersion(t *testing.T) {
...
func TestImportNAICSByFiles(t *testing.T) {
	ctx := context.Background()
	t.Run("both files are missing", func(t *testing.T) {
		_, err := importNAICSByFiles(ctx, "x", "xx")
		if err == nil {
			t.Error("importNAICSByFiles: did not return error when there should be an error")
		}
	})
	t.Run("one file exists while other file is missing", func(t *testing.T) {
		_, err := importNAICSByFiles(ctx, "./data/naics-scian-2022-element-v1-eng.csv", "./data/xxx.csv")
		if err == nil {
			t.Errorf("importNAICSByFiles: did not get error")
		}
	})
	t.Run("both file exists and verify output", func(t *testing.T) {
		// STEP 1: Lookup correct files.
		naicss, err := importNAICSByFiles(ctx, "./data/naics-scian-2022-element-v1-eng.csv", "./data/naics-scian-2022-structure-v1-eng.csv")
		if err != nil {
			t.Errorf("importNAICSByFiles: got error %v", err)
		}
		...
		for _, naics := range naicss {
			...
			if naics.CodeStr == "111110" {
				if naics.CanadianIndustryTitle != "Soybean farming" {
					t.Errorf("importNAICSByFiles: CanadianIndustryTitle is incorrect. Want %s but got %s", "Soybean farming", naics.CanadianIndustryTitle)
				}
				if naics.IndustryTitle != "Soybean farming" {
					t.Errorf("importNAICSByFiles: IndustryTitle is incorrect %v", naics.IndustryTitle)
				}
				if naics.IndustryGroupTitle != "Oilseed and grain farming" {
					t.Errorf("importNAICSByFiles: IndustryGroupTitle is incorrect %v", naics.IndustryGroupTitle)
				}
				if naics.SubsectorTitle != "Crop production" {
					t.Errorf("importNAICSByFiles: SubsectorTitle is incorrect %v", naics.SubsectorTitle)
				}
				if naics.SectorTitle != "Agriculture, forestry, fishing and hunting" {
					t.Errorf("importNAICSByFiles: MinorGroupTitle is incorrect %v", naics.SectorTitle)
				}
				if naics.Superscript != "" {
					t.Errorf("importNAICSByFiles: Superscript is incorrect %v", naics.Superscript)
				}
