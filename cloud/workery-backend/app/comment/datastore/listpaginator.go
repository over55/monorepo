...
// CommentPaginationListResult represents the paginated list results for
...
func (impl CommentStorerImpl) newPaginationFilter(f *CommentPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
		...
		default:
			return nil, fmt.Errorf("unsupported sort field for `%v`, only supported fields are `created_at` or `modified_at`", f.SortField)
		}
	}
	return bson.M{}, nil
}

func (impl CommentStorerImpl) newPaginationFilterBasedOnTimestamp(f *CommentPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl CommentStorerImpl) newPaginationOptions(f *CommentPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl CommentStorerImpl) newPaginatorNextCursor(f *CommentPaginationListFilter, results []*Comment) (string, error) {
	var lastDatum *Comment
	...
	case "modified_at":
		...
		break
	default:
		return "", fmt.Errorf("unsupported sort field in options for `%v`, only supported fields are `created_at` or `modified_at`", f.SortField)
	}
