...
func (impl *GatewayControllerImpl) ProfileUpdate(ctx context.Context, req *ProfileUpdateRequestIDO) error {
	...
	transactionFunc := func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		case u_d.UserRoleCustomer:
			if err := impl.ProfileUpdateCustomer(sessCtx, req, ou); err != nil {
				return nil, err
			}
			break
		case u_d.UserRoleAssociateJobSeeker:
			if err := impl.ProfileUpdateAssociateJobSeeker(sessCtx, req, ou); err != nil {
				return nil, err
			}
			break
		default:
...
func (impl *GatewayControllerImpl) ProfileUpdateCustomer(sessCtx mongo.SessionContext, req *ProfileUpdateRequestIDO, ou *u_d.User) error {
	...
	// STEP 1
	impl.Logger.Debug("validating associate user profile")
	if err := ValidateCustomerProfileUpdateRequest(req); err != nil {
		return err
	}
