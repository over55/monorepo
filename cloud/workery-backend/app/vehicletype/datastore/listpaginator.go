...
type VehicleTypePaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
...
type VehicleTypePaginationListResult struct {
...
func (impl VehicleTypeStorerImpl) newPaginationFilter(f *VehicleTypePaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl VehicleTypeStorerImpl) newPaginationFilterBasedOnString(f *VehicleTypePaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl VehicleTypeStorerImpl) newPaginationFilterBasedOnTimestamp(f *VehicleTypePaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl VehicleTypeStorerImpl) newPaginationOptions(f *VehicleTypePaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl VehicleTypeStorerImpl) newPaginatorNextCursor(f *VehicleTypePaginationListFilter, results []*VehicleType) (string, error) {
	var lastDatum *VehicleType
