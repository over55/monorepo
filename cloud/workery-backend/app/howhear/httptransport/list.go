...
function SettingHowHearAboutUsItemListPage() {
  ...
  const fetchItems = useCallback(async (cursor = currentCursor, resetStack = false) => {
    ...
    try {
      ...
      const queryParams = {
        cursor: cursor,
        page_size: pageSize, // Backend expects snake_case
        sort_field: sortField, // Already in snake_case
        sort_order: sortOrder, // Already a number
      };
          ...
          <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Role
                  <span className="text-xs text-gray-500 ml-1">(Frontend filter)</span>
                </label>
                <div className="relative">
                  <select
                    value={roleFilter}
