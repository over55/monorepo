...
func (impl OrderIncidentStorerImpl) generatePublicID(ctx context.Context, tenantID primitive.ObjectID) (uint64, error) {
	var publicID uint64
	latest, err := impl.GetLatestByTenantID(ctx, tenantID)
	if err != nil {
		impl.Logger.Error("database get latest orderincident by tenant id error",
			slog.Any("error", err),
			slog.Any("tenant_id", tenantID))
		return 0, err
	}
	if latest == nil {
		impl.Logger.Debug("first orderincident creation detected, setting publicID to value of 1",
			slog.Any("tenant_id", tenantID))
		publicID = 1
	} else {
		publicID = latest.PublicID + 1
		impl.Logger.Debug("system generated new orderincident publicID",
