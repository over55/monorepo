...
type Staff struct {
	...
	// OTPEnabled controls whether we force 2FA or not during login for this staff member.
...
type StaffStorer interface {
	...
	GetByUserID(ctx context.Context, userID primitive.ObjectID) (*Staff, error)
	...
	CountByFilter(ctx context.Context, f *StaffPaginationListFilter) (int64, error)
	ListAndCountByFilter(ctx context.Context, f *StaffPaginationListFilter) (*StaffPaginationListAndCountResult, error)
	LiteListAndCountByFilter(ctx context.Context, f *StaffPaginationListFilter) (*StaffPaginationLiteListAndCountResult, error)
...
func NewDatastore(appCfg *c.Conf, loggerp *slog.Logger, client *mongo.Client) StaffStorer {
	...
	if err != nil {
		log.Println("index problem at staff")
