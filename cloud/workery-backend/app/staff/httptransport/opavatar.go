...
func UnmarshalOperationAvatarRequest(ctx context.Context, r *http.Request) (*c_c.StaffOperationAvatarRequest, error) {
	defer r.Body.Close()
	...
	staffID, err := primitive.ObjectIDFromHex(staffIDStr)
	if err != nil {
		log.Println("UnmarshalCmsImageCreateRequest: primitive.ObjectIDFromHex:err:", err)
	}

	// Initialize our array which will store all the results from the remote server.
	requestData := &c_c.StaffOperationAvatarRequest{
		StaffID: staffID,
	}
	if header != nil {
...
func MarshalOperationAvatarResponse(res *c_s.Staff, w http.ResponseWriter) {
