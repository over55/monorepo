...
func (impl ServiceFeeStorerImpl) newPaginationFilter(f *ServiceFeePaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl ServiceFeeStorerImpl) newPaginationFilterBasedOnString(f *ServiceFeePaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl ServiceFeeStorerImpl) newPaginationFilterBasedOnTimestamp(f *ServiceFeePaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl ServiceFeeStorerImpl) newPaginationOptions(f *ServiceFeePaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl ServiceFeeStorerImpl) newPaginatorNextCursor(f *ServiceFeePaginationListFilter, results []*ServiceFee) (string, error) {
	var lastDatum *ServiceFee
