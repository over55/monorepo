...
func (impl *TaskItemControllerImpl) validateCloseOperationRequest(ctx context.Context, dirtyData *TaskItemOperationCloseRequestIDO) error {
	...
	} else if dirtyData.WasCompleted == 1 {
		if dirtyData.CompletionDate == "" {
			e["completion_date"] = "missing value"
		}
	} else if dirtyData.WasCompleted == 2 {
...
func (impl *TaskItemControllerImpl) CloseOperation(ctx context.Context, req *TaskItemOperationCloseRequestIDO) (*ti_s.TaskItem, error) {
	////
	//// Perform validation and security checks.
	////

	if err := impl.validateCloseOperationRequest(ctx, req); err != nil {
	...
	transactionFunc := func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		impl.Logger.Debug("task item closed due to closure",
			slog.Any("task_item_id", req.TaskItemID))

		////
		//// Create a new task based on `WasCompleted` field.
		////
		...
		if req.WasCompleted == 1 { // --------- Success --------- //
			...
			description := "Please call client and review the associate."
			dueDate := time.Now().AddDate(0, 0, 7)                                      // Add 7 day. Rational: We want to ask the customer after 7 days AFTER the client meeting data.
			typeId := int8(ti_s.TaskItemTypeFollowUpDidCustomerReviewAssociateAfterJob) // A.K.A. "Survey" type.
		...
		} else if req.WasCompleted == 2 { // --------- Failure --------- //
			////
			//// Update existing order.
			////
