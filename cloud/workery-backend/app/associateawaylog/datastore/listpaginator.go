...
func (impl AssociateAwayLogStorerImpl) newPaginationFilter(f *AssociateAwayLogPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
		...
		default:
			return nil, fmt.Errorf("unsupported sort field for `%v`, only supported fields are `start_date` and `created_at`", f.SortField)
		}
	}
	return bson.M{}, nil
}

func (impl AssociateAwayLogStorerImpl) newPaginationFilterBasedOnString(f *AssociateAwayLogPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl AssociateAwayLogStorerImpl) newPaginationFilterBasedOnTimestamp(f *AssociateAwayLogPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl AssociateAwayLogStorerImpl) newPaginationOptions(f *AssociateAwayLogPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl AssociateAwayLogStorerImpl) newPaginatorNextCursor(f *AssociateAwayLogPaginationListFilter, results []*AssociateAwayLog) (string, error) {
	var lastDatum *AssociateAwayLog
	...
	case "start_date":
		...
		break
	default:
		return "", fmt.Errorf("unsupported sort field in options for `%v`, only supported fields are `start_date` and `created_at`", f.SortField)
	}
