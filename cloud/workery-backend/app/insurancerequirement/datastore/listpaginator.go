...
func (impl InsuranceRequirementStorerImpl) newPaginationFilter(f *InsuranceRequirementPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl InsuranceRequirementStorerImpl) newPaginationFilterBasedOnString(f *InsuranceRequirementPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl InsuranceRequirementStorerImpl) newPaginationFilterBasedOnTimestamp(f *InsuranceRequirementPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl InsuranceRequirementStorerImpl) newPaginationOptions(f *InsuranceRequirementPaginationListFilter) (*options.FindOptions, error) {
	...
	if f.SortField != "" {
		sortSpec := bson.D{{f.SortField, f.SortOrder}}

		// Only add _id if it's not already the sort field to avoid duplicate keys
		if f.SortField != "_id" {
			sortSpec = append(sortSpec, bson.E{"_id", f.SortOrder})
		}

		options = options.SetSort(sortSpec)
	}
...
// with our paginator.
func (impl InsuranceRequirementStorerImpl) newPaginatorNextCursor(f *InsuranceRequirementPaginationListFilter, results []*InsuranceRequirement) (string, error) {
	var lastDatum *InsuranceRequirement
