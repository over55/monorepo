...
// ActivitySheetPaginationListResult represents the paginated list results for
// the activity sheet records.
...
// ActivitySheetPaginationLiteListResults represents the paginated list results
// for the activity sheet records.
...
func (impl ActivitySheetStorerImpl) newPaginationFilter(f *ActivitySheetPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
		...
		case "associate_lexical_name":
			// STEP 3: Build for `associate_lexical_name` field.
			return impl.newPaginationFilterBasedOnString(f, string(decodedCursor))
		case "created_at":
			// STEP 3: Build for `created_at` field.
			return impl.newPaginationFilterBasedOnTimestamp(f, string(decodedCursor))
		default:
			return nil, fmt.Errorf("unsupported sort field for `%v`, only supported fields are `associate_lexical_name` and `created_at`", f.SortField)
		}
	}
	return bson.M{}, nil
}

func (impl ActivitySheetStorerImpl) newPaginationFilterBasedOnString(f *ActivitySheetPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl ActivitySheetStorerImpl) newPaginationFilterBasedOnTimestamp(f *ActivitySheetPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl ActivitySheetStorerImpl) newPaginationOptions(f *ActivitySheetPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl ActivitySheetStorerImpl) newPaginatorNextCursor(f *ActivitySheetPaginationListFilter, results []*ActivitySheetLite) (string, error) {
	var lastDatum *ActivitySheetLite
	...
	case "created_at":
		...
		break
	default:
		return "", fmt.Errorf("unsupported sort field in options for `%v`, only supported fields are `associate_lexical_name` and `created_at`", f.SortField)
	}
...
// newPaginatorNextCursorForFull will return the base64 encoded next cursor
// which works with our paginator.
func (impl ActivitySheetStorerImpl) newPaginatorNextCursorForFull(f *ActivitySheetPaginationListFilter, results []*ActivitySheet) (string, error) {
	var lastDatum *ActivitySheet
