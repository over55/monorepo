	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if ok && isAuthorized {
			...
			if user.OTPEnabled && user.OTPVerified && !user.OTPValidated {
				...
				// Check to see if the user is calling `/api/v1/otp/validate`
				// or `/api/v1/otp/recovery` API endpoints.
				if len(urlSplit) > 3 && urlSplit[2] == "otp" && (urlSplit[3] == "validate" || urlSplit[3] == "recovery") {
			...
			ctx = context.WithValue(ctx, constants.SessionUserReferenceID, user.ReferenceID)
		}
...
// the user is not authorized when visiting a protected URL. Note: This middleware must have
// `IPAddressMiddleware` executed first before running.
